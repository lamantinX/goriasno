...
| Verdict | When | Action |
|---|---|---|
| **APPROVE** | Criteria pass, scope clean, quality holds | Update index status to DONE. Push the branch to remote and create a Pull Request via `gh pr create`. Present the PR link to the user. **Merging the PR is the user's decision.** |
